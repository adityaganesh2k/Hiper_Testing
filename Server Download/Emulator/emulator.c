...
#define UPPER_TIMER_LIMIT 135
#define LOWER_TIMER_LIMIT 15
#define WAIT_TIMER 10
	...
	//Timer
		...
		uint32_t turn_off = 0, wait_time = 0, timeout = 0;

	uint8_t trip_complete = 0;
	...
	*/
	...
	void TIM7_IRQHandler()
	{
		...
		if(timeout == turn_off)
		{
			...
			//Disable all messages being sent
	...
	*/
	void TIM6_DAC_IRQHandler()
	{
		...
		if(wait_time == WAIT_TIMER)
		{
			trip_complete = 0;
			wait_time = 0;
			srand(i);
			timeout = rand()%(UPPER_TIMER_LIMIT - LOWER_TIMER_LIMIT + 1) + LOWER_TIMER_LIMIT;
			...
			//Enable all messages being sent
