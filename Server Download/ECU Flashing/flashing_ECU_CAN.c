...
//Variables
	...
	// Version Control
		uint8_t pay_ver, cmd_ver, ecu_ver;
	...
	//FATFS Variables
		FATFS fs;            	  	/* Filesystem object */
   		FIL file, boot_config, bin;           			/* File object */
		...
		uint8_t dtc_read = 0, filename_flag = 0, set_rpm = 0, flashing_complete = 0;
		char filename[15]="t_";
	...
	void new_file()
	{
		...
		do
		{
			...
			switch (res)
			{
				...
				case FR_NO_FILE:
					res = f_open(&bin, filename, FA_CREATE_NEW | FA_WRITE);
					filename_flag = 1;
				break;
	...
	*/
	void load_cmd()
	{
		...
		//Open required file
			res= f_open(&file, "cmd.csv", FA_OPEN_EXISTING|FA_READ);
	...
	//Read the total # of areas to be flashed
			memset(temp_buff,0,sizeof(temp_buff));
			memset(s,0,sizeof(s));
			f_gets (temp_buff,70,&file);
			k=1;
			...
			bin_area_size=atoi(s);				//total Number of  areas
			memset(s,0,sizeof(s));
			memset(temp_buff,0,sizeof(temp_buff));
			k=1;
			i=0;
	...
	//Load Baud Change details
		while(j<bin_area_size)
		{
			memset(temp_buff,0,sizeof(temp_buff));
			memset(s,0,sizeof(s));
			f_gets(temp_buff,70,&file);
			...
			while(k)
			{
				if(temp_buff[i] == ',') //Process buffer s
				{
					switch(m)
					{
						case 0:							// Read Start Address of Area j in hex
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].start_address);
							memset(s,0,sizeof(s));
							break;
						}
						case 1:							// Read End address of Area j in hex
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].finish_address);
							memset(s,0,sizeof(s));
							break;
						}
						case 2:							// Read end frame number till which Full CS is calculated
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].checksum_end_frame);
							memset(s,0,sizeof(s));
							break;
						}
						case 3:							// Read file pointer location of area j
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].payload_pointer);
							memset(s,0,sizeof(s));
							break;
						}
						case 4:							// Read response counter to which control goes to
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].start_response_counter);
							memset(s,0,sizeof(s));
							break;
						}
						case 5:							// Read end frame number
						{
							s[n]='\0';
							sscanf(s,"%x",&bin_area_task[j].end_frame);
							memset(s,0,sizeof(s));
							k=0;
							break;
						}
	...
	*/
	...
	void Edc16uc40Algo(uint32_t dwSeed, uint32_t* pdwKey)
	{
	...
	*/
	...
	/* Check if received message is same as expected message */ 
		if(can_process_flag)
		{
			...
			if(dtc_read)
			{
				res = f_write(&bin, CAN_Frame, buff_length, &bw);
				f_sync(&bin);
			}
		...
		// Process the response
			if(cmd_status_flag == 2)	// If expected replies
			{
				...
				{
					case 1:				//Normal Replies
					{
						if (cmd_resp[cmd_resp_counter-1].type == 4)
						{
							frames_confirmed_sent++;
							pay_sent_bytes += (Active_TxFlag == 1) ?  payload_size_1 : payload_size_2;

							if (Active_TxFlag==1)
								tx1_empty_flag=1;
							else if (Active_TxFlag==2)
								tx2_empty_flag=1;
							Active_TxFlag=3;
							if(frame_number_1== frames_confirmed_sent)
								Active_TxFlag=1;
							else if(frame_number_2 == frames_confirmed_sent)
								Active_TxFlag=2;

							if(pay_sent_bytes == (bin_area_task[bin_area_counter].finish_address - bin_area_task[bin_area_counter].start_address) + 0x1 )
							{
								if(frames_confirmed_sent == bin_area_task[bin_area_counter].end_frame+1 )
									cmd_resp_counter++;
								else
									full_checksum_error = 21;
							}

							if(frames_confirmed_sent > bin_area_task[bin_area_counter].end_frame + 1)
								full_checksum_error = 21;
						}
						else if (cmd_resp[cmd_resp_counter-3].type == 6)
						{
							payload_read=0;
							full_checksum=0;
							pay_sent_bytes=0;
							...
							pay_check_read=0;
							pay_check_calc=0;
							full_checksum_error=0;
							bin_area_counter++;
						}
						break;
					}
					case 2:				//Replies with Seed for security access
					{
						seed = 0;
						response = 0;
						for (int r=0; r<4;r++)
						{
							seed= seed+((rx_can.Data[cmd_resp[cmd_resp_counter].size+r])<<(8*(3-r)));
						}
						Edc16uc40Algo(seed,&response);
						for (int r=0; r<4;r++)
						{
							cmd_resp[cmd_resp_counter+1].message[cmd_resp[cmd_resp_counter+1].size+r]=(0xFF)&(response>>(8*(3-r)));
						}
						cmd_resp[cmd_resp_counter+1].size = 7;
						break;
					}
					case 8:
					{
					...
					case 9:
					{
						if (cmd_resp[cmd_resp_counter-3].type == 6)
						{
							payload_read=0;
							full_checksum=0;
							pay_sent_bytes=0;
					...
					case 10:
					{
						...
						{
							// Turn on engine for 1 min
							ESP_CMD_SEND(TURN_VEHCILE_ON);
							dtc_read = 1;
						}
						break;
					}
				...
				if((cmd_resp[cmd_resp_counter-2].type != 4))
				{
			...
			{
				if (cmd_resp[cmd_resp_counter].type == 9)
				{
					cmd_resp_counter=cmd_resp[cmd_resp_counter].failure_index;
					wait=1;
					NVIC_DisableIRQ(TIM6_DAC_IRQn);
					if(cmd_resp[cmd_resp_counter-2].type == 6)
					{
						full_checksum_error++;
						// if (full_checksum_error>=6)
						// 	{
						// 		cmd_resp_counter=cmd_resp[cmd_resp_counter].sucess_index;
						// 		cmd_resp_counter=cmd_resp[cmd_resp_counter].sucess_index;
						// 	}
			...
			//Activate TIM6 to send the next cmd
				...
				if(!wait_30s && (cmd_resp[cmd_resp_counter].type != 8))
				{
					if(cmd_resp[cmd_resp_counter].type == 4)
					{
						if(Active_TxFlag != 3)
						{
							timer_mode=1;
	...
	void USART3_IRQHandler()
	{
		...
		if(esp_ch == '@')
		{
			if(!flashing_complete)
				ESP_CMD_SEND(KEEP_IGN_ON);
			else
				ESP_CMD_SEND(FLASHING_COMPLETE);
		}
	...
	*/	
	void TIM6_DAC_IRQHandler()                 //IRQ FOR TIMER=25ms
	{
		...
		if(cmd_resp[cmd_resp_counter].type==4)
		{
			...
			if (Active_TxFlag == 1)
			{
				if(Tx1_num_frames > 1)
				{
					Send_Multiframes(Data1, Tx1_num_frames);
					/*
						send_multiframes = 2; 
						uint8_t Data_Buff_num;
						Data_Buff_num = 1;
			...
			{
				if(Tx2_num_frames > 1)
					Send_Multiframes(Data2, Tx2_num_frames);
				else
				{
					for(int i=0; i<8; i++)
						tx_can.Data[i] = Data4[i];
		...
		else if( (cmd_resp[cmd_resp_counter].type==0) ||(cmd_resp[cmd_resp_counter].type==3) )
		{
			if(cmd_resp[cmd_resp_counter].size > 7)					
			{
		...
		else if(cmd_resp[cmd_resp_counter].type == 6)
		{
			/*// first separate full checskusm into 2 bytes
			// add it to the msg along with checsum for that msg
			cmd_resp[cmd_resp_counter].message[10] = (full_checksum & 0xFF00) >> 8;
			cmd_resp[cmd_resp_counter].message[11] = (full_checksum & 0xFF);
			cmd_resp[cmd_resp_counter].size=12;
			*/
			obj = CAN_PACK_MULTIFRAMES(Data3, cmd_resp[cmd_resp_counter].message, cmd_resp[cmd_resp_counter].size);	// Define it. packing_can file
	...
	{		
		...
		if(wait_30s == 1)
		{
			if(set_rpm && can_inactive > 20)			// Start sending RPM Set commands
			{
				wait_30s = 0;
				can_inactive = 0;
				NVIC_EnableIRQ(TIM6_DAC_IRQn);
			...
			else if(dtc_read && can_inactive > 200)		// Wait for 100s - ENG ON after rpm change and send DTC
			{
				wait_30s = 0;
				can_inactive = 0;
				ESP_CMD_SEND(TURN_VEHCILE_ON);				// TURN OFF ENG
		...
		else if((!wait_30s) && can_inactive > 4)
		{
			if(wait == 0)
			{
				// Retransmit messages
  ...
  /*
		Using Push Button as a kick start for ECU Flashing
	*/
...
int main()
{
	...
	//Open required file
		res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
	...
	//CAN 
		...
		// 	if(start_music)
		// 	{
		// 		// Press button to start flashing
		// 		start_music = 0;
		// 		ESP_CMD_SEND();
		// 	}
		...
		TIM7_Base_Start_IT();
		int q = 0;
		char config_buffer[20], Update_payload;
	while(1)
    {
		if(LED_ON==1)
		{
		...
		else if(LED_ON==7)
		{
			...
			res = f_write(&boot_config, config_buffer,12, &bw);
			...
			ecu_ver = atoi(config_buffer);

			f_close(&boot_config);

			res = f_open(&boot_config, "ecu_ver.csv", FA_WRITE|FA_READ);

			memset(config_buffer,0,sizeof(config_buffer));
			sprintf(config_buffer, "BLANK\n%02d\n%02d\n%02d\n",pay_ver, cmd_ver, ecu_ver);
			res = f_write(&boot_config, config_buffer,15, &bw);
		...
		if (full_checksum_error>20)
		{
			...
			frame_number_1 =0;
			frame_number_2 =0;
			if(bin_area_counter==0)
				frames_confirmed_sent=0;
			else
				frames_confirmed_sent = bin_area_task[bin_area_counter-1].end_frame;
			cmd_resp_counter = bin_area_task[bin_area_counter].start_response_counter;
			f_close(&file);
			res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
			res = f_lseek(&file, bin_area_task[bin_area_counter].payload_pointer);
			Active_TxFlag=3;

		}

		while(tx1_empty_flag == 1 &&
				(payload_read < (bin_area_task[bin_area_counter].finish_address - bin_area_task[bin_area_counter].start_address + 0x1) ) &&
				(frame_number_1 <bin_area_task[bin_area_counter].end_frame ))
		{
			struct ret_val obj;

			// Empty KlineTxbuff1 and Prepare packet and store in txbuff1
			...
			pay_check_calc=0;
			frame_number_1=0;
			payload_pointer = f_tell(&file);
			task_status =0;
			memset (temp_buf_frame[2],0,2);
			while(task_status!=1)
			{
				switch(f_read(&file, &temp_buf_frame, 2, &br))
				{
					case FR_OK:
					{
						task_status=1;
					...
					{
						f_close(&file);
						memset(temp_buf_frame,0,sizeof(temp_buf_frame));
						res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
						res = f_lseek(&file, payload_pointer);
						break;
					}
				}
			}
			frame_number_temp= (0xFF00 & temp_buf_frame[0]<<8)+  temp_buf_frame[1];
 			payload_pointer = f_tell(&file);
			task_status =0;
			while(task_status!=1)
			{
				switch(f_read(&file, &payload_size_1, 1, &br))
				{
					case FR_OK:
					{
						task_status=1;
					...
					{
						f_close(&file);
						memset(payload_size_1,0,sizeof(payload_size_1));
						res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
						res = f_lseek(&file, payload_pointer);
						break;
					}
			...
			while(task_status!=1)
			{
				switch(f_read(&file, &temp_buf[1], payload_size_1, &br))
				{
					case FR_OK:
					{
						task_status=1;
			...
			while(task_status!=1)
			{
				switch(f_read(&file, &pay_check_read, 1, &br))
				{
					case FR_OK:
					{
						task_status=1;
					...
					{
						f_close(&file);
						memset(pay_check_read,0,sizeof(pay_check_read));
						res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
						res = f_lseek(&file, payload_pointer);
						break;
					}
			...
			{				
				temp_buf[0] = 0x36;
				obj = CAN_PACK_MULTIFRAMES(Data1, &temp_buf, payload_size_1+1);
				pay_check_calc = obj.checksum;
				Tx1_num_frames = obj.num_frames;
				pay_check_calc -= 0x36;
			}
			else
			{
				Data4[0] = payload_size_1+1;
				Data4[1] = 0x36;

				for(int i=1; i<=payload_size_1; i++)
				{
					pay_check_calc += temp_buf[i];
					Data4[i+1] = temp_buf[i];
				}
				Tx1_num_frames = 1;
			}
			
			if((uint8_t)pay_check_calc == pay_check_read)
			{
				payload_read+= payload_size_1;
                frame_number_1=frame_number_temp;
				if(frame_number_1 <= bin_area_task[bin_area_counter].checksum_end_frame)
					full_checksum += pay_check_calc;
			...
			{
				memset(temp_buf,0,sizeof(temp_buf));
				res = f_lseek(&file, f_tell(&file) - payload_size_1 - 4);
			}

		}
		while(tx2_empty_flag == 1 &&
				(payload_read < (bin_area_task[bin_area_counter].finish_address - bin_area_task[bin_area_counter].start_address + 0x1) ) &&
				(frame_number_2 < bin_area_task[bin_area_counter].end_frame ) &&
				ensure_1 )
		{
			struct ret_val obj;

			// Prepare packet and store in txbuff2
			memset(temp_buf,0,sizeof(temp_buf));
			payload_size_2=0;
			pay_check_read=0;
			Tx2_num_frames=0;
			...
			while(task_status!=1)
			{
				switch(f_read(&file, &payload_size_2, 1, &br))
				{
					case FR_OK:
					{
						task_status=1;
					...
					{
						f_close(&file);
						memset(payload_size_2,0,sizeof(payload_size_2));
						res= f_open(&file, "payload.bin", FA_OPEN_EXISTING|FA_READ);
						res = f_lseek(&file, payload_pointer);
						break;
					}
			...
			while(task_status!=1)
			{
				switch(f_read(&file, &temp_buf[1], payload_size_2, &br))
				{
					case FR_OK:
					{
						task_status=1;
			...
			{
				temp_buf[0] = 0x36;
				obj = CAN_PACK_MULTIFRAMES(Data2, &temp_buf, payload_size_2+1);
				pay_check_calc = obj.checksum;
				Tx2_num_frames = obj.num_frames;
				pay_check_calc -= 0x36;
			}
			else
			{
				Data4[0] = payload_size_2+1;
				Data4[1] = 0x36;

				for(int i=1; i<=payload_size_2; i++)
				{
					pay_check_calc += temp_buf[i];
					Data4[i+1] = temp_buf[i];
				}
			...
			if((uint8_t)pay_check_calc == pay_check_read)
			{
				payload_read+= payload_size_2;
                frame_number_2=frame_number_temp;
				if(frame_number_2 <= bin_area_task[bin_area_counter].checksum_end_frame)
					full_checksum += pay_check_calc;
			...
			{
				memset(temp_buf,0,sizeof(temp_buf));
				res = f_lseek(&file, f_tell(&file) - payload_size_2 - 4);
			}
