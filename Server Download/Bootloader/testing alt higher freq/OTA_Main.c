...
#define UPLOAD_TIMER_TIMEOUT 1000
	...
	//Load DBC
	uint8_t data_log_mode, num_can_ids, response_fifo, can_speed, num_pgns, fifo1_pgn, copy_of_data_log_mode;
	...
	void update_rb()
	{
		...
		if(rb_flag==1)
		{
			...
			if(gps_log_flag)
			{
				...
				//Log the Altitude
					memcpy(temp_buf,z.altitude1,6);
					strncat(temp_buf,",",1);
					strncat(rb1,temp_buf,7);
					rb1_size=rb1_size+7;
					memset(temp_buf,0,sizeof(temp_buf));

					memcpy(temp_buf,z.altitude2,6);
					strncat(temp_buf,",",1);
					strncat(rb1,temp_buf,7);
					rb1_size=rb1_size+7;
					memset(temp_buf,0,sizeof(temp_buf));

					memset(z.altitude1,'0',6);
					memset(z.altitude2,'0',6);
			}
			else
			{
				strncat(rb1,",,,,",4);
	...
	*/
	...
	void startup()
	{
		...
		//GPS
			...
			memset(z.altitude1,'0',sizeof(z.altitude1));
			memset(z.altitude2,'0',sizeof(z.altitude2));
		//Initiate the DAQ module 
			init_daq();
...
*/
	while(1)
	{
		...
		if(data_log_mode != copy_of_data_log_mode || data_log_mode > 3)
		{
			memset(file_ending,0,sizeof(file_ending));
