...
void TIM7_IRQHandler() //IRQ FOR TIMER=500ms
{
	...
	case 1:
		...
		else if (ENG_RUNNING && IGN_ON)
		{
			...
			case 1:
			{
				if ((Data[broadcast_params][0] == 0x0 && Data[broadcast_params][1] == 0x0) || (Data[broadcast_params][0] == 0xFF && Data[broadcast_params][1] == 0xFF))
				{
					stop_logging++;
				}
				else
...
void USART3_IRQHandler()
{
	...
	if (!IFT_Start)
	{
		...
		//Connected to hotspot so upload can begin
		case '&':
		{
			memcpy(wh[wh_counter].time, z.time, 6);
