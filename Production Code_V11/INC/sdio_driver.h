#ifndef _SDIO_DRIVER_H
#define _SDIO_DRIVER_H
...
/* Macros to ENABLE clock for SDIO using RCC Register */
#define RCC_SDIO_CLOCK_ENABLE_CLK()   (RCC->APB2ENR |= (1<<11))


/* Macros to ENABLE clock SDIO using RCC Register in LOW POWER Mode */
#define RCC_SDIO_CLOCK_ENABLE_LP_CLK()   (RCC->APB2LPENR |= (1<<11))
...
typedef struct {
	...
	uint32_t    BlockSize;       // SD card block size (bytes), determined in CSD
	...
	uint8_t     SCR1[8];         // SD card SCR register (SD card configuration)
} SDCard_TypeDef;
